...
#### 批量导入读者
- 准备批量导入模板: 使用Excel（电子表格）软件，创建一个电子表格文件，在第1行前5个单元格输入表头名称: userId(读者号),(姓名),gender(男生写1女生写2),classInfo(对应班级管理中的班级名称),borrowRestriction(对应身份管理中的身份名称)
...
- 将读者信息的csv文件传到平板/手机上，通过"读者管理"->从文件导入功能来选择该csv文件进行批量导入。
